# Bi-channel Image Registration and Deep-learning Segmentation (BIRDS)
# Xuechun Wang, Weilin Zeng
# This code is optimized from the deeplab v3 plus (Waleed Abdulla, (c) 2017 Matterport, Inc.) repository
...
def _build_birds(iterator, outputs_to_num_classes, ignore_label):
  """Builds a clone of birds.
...
def _tower_loss(iterator, num_of_classes, ignore_label, scope, reuse_variable):
  """Calculates the total loss on a single tower running the birds model.

  Args:
    iterator: An iterator of type tf.data.Iterator for images and labels.
    num_of_classes: Number of classes for the dataset.
    ignore_label: Ignore label for the dataset.
    scope: Unique prefix string identifying the birds tower.
  ...
  with tf.variable_scope(
    ...
    _build_birds(iterator, {common.OUTPUT_TYPE: num_of_classes}, ignore_label)
...
def _train_birds_model(iterator, num_of_classes, ignore_label):
  """Trains the birds model.
...
def main(unused_argv):
  ...
  with graph.as_default():
    with tf.device(tf.train.replica_device_setter(ps_tasks=FLAGS.num_ps_tasks)):
      ...
      train_tensor, summary_op = _train_birds_model(
